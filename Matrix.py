...
class Matrix:
    ...
    def __add__(self, other):
        ...
        result_values = [[self.Values[i][j] + other.Values[i][j] for j in range(self.CountColumns)] for i in range(self.CountRows)]
        return Matrix(rows=self.CountRows, columns=self.CountColumns, values=result_values)

    def __mul__(self, other):
        if self.CountColumns != other.CountRows:
            raise ArithmeticError("Number of columns in the first matrix should be equal to the number of rows in the second matrix")
        result_values = [[sum(a * b for a, b in zip(row, col)) for col in zip(*other.Values)] for row in self.Values]
